...
persona: A
prerequisites: []
next: [schema-authoring]
---

# DBSCHEMA_COMPONENT_SKILL
> `jardistools/dbschema` | NS: `JardisTools\DbSchema` | PHP 8.2+ | Infrastructure only — Domain NEVER imports DbSchema.
...
$reader->columns(string $container, ?array $fields = null): array  // filtered + ordered by $fields
$reader->indexes(string $table): array
$reader->foreignKeys(string $table): array
$reader->fieldType(string $dbType): ?string           // e.g. 'varchar' → 'string'; null if type unmapped
...
```
`tables()`/`columns()`/`indexes()`/`foreignKeys()` return `[]` when no results — never `null`. `fieldType()` returns `?string` — `null` when the DB type is unmapped.
