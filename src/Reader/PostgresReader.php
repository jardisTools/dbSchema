 ...
 */
class PostgresReader implements DbSchemaReaderInterface
{
    private PDO $pdo;
    ...
    public function foreignKeys(string $table): ?array
    {
        $sql = <<<SQL
            SELECT
                cl.relname                          AS "container",
                c.conname                           AS "constraintName",
                a_local.attname                     AS "constraintCol",
                cl_ref.relname                      AS "refContainer",
                a_ref.attname                       AS "refColumn",
                n_ref.nspname                       AS "refSchema",
                CASE c.confupdtype
            ...
            CROSS JOIN LATERAL unnest(c.conkey, c.confkey)
                WITH ORDINALITY AS u(local_attnum, ref_attnum, ordinality)
            JOIN pg_attribute a_local
                ON a_local.attrelid = c.conrelid
                AND a_local.attnum = u.local_attnum
            JOIN pg_attribute a_ref
                ON a_ref.attrelid = c.confrelid
                AND a_ref.attnum = u.ref_attnum
            WHERE c.contype = 'f'
              AND n.nspname = 'public'
              AND cl.relname = :table
            ORDER BY c.conname, u.ordinality
        SQL;
