...
class MySqlDialect implements DdlDialectInterface
{
    public function typeMapping(string $dbType, ?int $length, ?int $precision, ?int $scale): string
    {
        ...
        return match ($type) {
            ...
            'DECIMAL', 'NUMERIC' => $precision !== null && $scale !== null
                ? $type . '(' . $precision . ',' . $scale . ')'
                : ($precision !== null ? $type . '(' . $precision . ')' : $type),
    ...
    /**
     * Quote a default value appropriately for MySQL DDL.
     *
     * SQL keywords (CURRENT_TIMESTAMP, NULL, TRUE, FALSE) and numeric literals
     * are emitted unquoted; string values are single-quoted.
     */
