...
namespace JardisTools\DbSchema\Exporter\Ddl;
        ...
        foreach ($sortedTables as $tableName) {
            ...
            if (!empty($data['foreignKeys'])) {
                foreach ($data['foreignKeys'] as $fk) {
                    $statement = $this->dialect->createForeignKeyStatement($tableName, $fk);
                    if (!empty($statement)) {
                        $output[] = $statement;
                    }
                }
...
-- SQL DDL Export
-- Generated: {$timestamp}
-- Exporter: JardisTools\DbSchema\Exporter\SqlDdlExporter
