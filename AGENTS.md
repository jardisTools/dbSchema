...
- **`tables()`, `columns($table, ?$fields)` (order respected when `$fields` is given), `indexes($table)`, `foreignKeys($table)` always return arrays, never `null`** — `[]` on empty results. `fieldType(string $dbType): ?string` maps DB types to PHP types (`varchar`/`text` → `string`, `int`/`bigint` → `int`, `decimal`/`float` → `float`, `boolean` → `bool`, MySQL/PG `JSON` → `array`, SQLite `json` → `string`) and returns `null` when the DB type is unmapped.
